        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className="relative flex h-12 w-12 items-center justify-center rounded-full transition-all duration-200 group"
            >
              {isActive && (
                <motion.div
              ...
              <div className="relative">
                <Icon className={`h-6 w-6 transition-all duration-200 ${isActive ? "text-white scale-110" : "text-white/30 group-hover:text-white/60"}`} />
                {tab.hasBadge && (
