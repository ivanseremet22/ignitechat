...
export function subscribeToReadReceipts(
  client: SupabaseClient,
  conversationId: string,
  onRead: (payload: { viewerId: string; lastReadAt: string; conversationId: string }) => void,
): { notifyRead: (viewerId: string, lastReadAt: string) => void; unsubscribe: () => void } {
  let channel: RealtimeChannel | null = null;
  ...
  try {
    channel = client
      ...
      .on("broadcast", { event: "read" }, ({ payload }) => {
        const data = payload as { viewerId?: string; lastReadAt?: string; conversationId?: string } | null;
        if (!data?.viewerId || !data?.lastReadAt || data.conversationId !== conversationId) return;
        onRead({
    ...
    console.error("subscribeToReadReceipts error:", error);
  }

  return {
    notifyRead(viewerId: string, lastReadAt: string) {
      void channel?.send({
        type: "broadcast",
        event: "read",
        payload: {
